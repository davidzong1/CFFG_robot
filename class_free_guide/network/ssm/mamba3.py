...
# from mamba_selective_scan import selective_scan_fn  # type: ignore
...
class InferenceCache(NamedTuple):
    ...
    def alloc(batch_size: int, args: Mamba3Config, device: Device = None):
        return InferenceCache(
            ssm_state=torch.zeros(batch_size, args.nheads, args.headdim, args.d_state, device=device),
            prev_Bx=torch.zeros(batch_size, args.nheads, args.headdim, args.d_state, device=device),
            cum_angle=torch.zeros(batch_size, 1, args.d_state // 2, device=device),
        )
                    ...
                    [
                        nn.ModuleDict(
                            dict(
                                ...
                                mixer=Mamba3Block(args, device=device),
...
class Mamba3Block(nn.Module):
    ...
    def forward(self, u: Tensor, h: InferenceCache | None = None):
        ...
        raw_angles = dt.unsqueeze(-1) * rearrange(  # (b, l, nheads, 1)
            theta, "b l n -> b l 1 n"
        )  # (b, l, 1, d_state//2)  # Shape: (batch, seqlen, nheads, d_state // 2)
    ...
    def step(self, u: Tensor, h: InferenceCache) -> tuple[Tensor, InferenceCache]:
        ...
        raw_angle = dt.unsqueeze(-1) * theta.unsqueeze(1)  # (batch, nheads, 1)  # (batch, 1, d_state//2)  # (batch, nheads, d_state//2)
