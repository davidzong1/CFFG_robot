...
class SelfAttentionBlock(nn.Module):
    def __init__(
        ...
        self._WK = nn.Linear(hidden_dim, num_attention_heads * self.head_dim)
        self._WV = nn.Linear(hidden_dim, num_attention_heads * self.head_dim)

        # 输出投影
        self._WO = nn.Linear(num_attention_heads * self.head_dim, hidden_dim)

    def forward(self, hidden_input, causal_mask=None):
        """
        Args:
            hidden_input: (batch,token_length, seq_len, hidden_dim)
            causal_mask: (batch, 1, seq_len, seq_len) 可选的因果掩码
        """
        ...
        K = self._WK(hidden_input).view(batch_size, seq_len, self.num_attention_heads, self.head_dim).transpose(1, 2)
        V = self._WV(hidden_input).view(batch_size, seq_len, self.num_attention_heads, self.head_dim).transpose(1, 2)
...
class SelfTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.cross_attention(norm_hidden_states_pos, causal_mask=attention_mask)
