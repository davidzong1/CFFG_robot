...
def _cfg_get(cfg: dict | object, key: str, default: Any = None) -> Any:
    """Read *key* from a dict-like or object-like *cfg*.

    Tries ``cfg.get(key, default)`` first (dict), then falls back to
    ``getattr(cfg, key, default)`` (dataclass / object).
    """
...
class Logger:
    ...
    def __init__(
        ...
        log_dir: str | None,
        cfg: dict | object,
        env_cfg: dict | object | None,
        num_envs: int,
        is_distributed: bool,
        gpu_world_size: int,
        gpu_global_rank: int,
        device: str,
        git_status_repos: list[str] | None = None,
        num_steps_per_env: int | None = None,
    ) -> None:
        """Initialize buffers and logging state for a training run.

        Args:
            cfg: Training configuration (dict or dataclass).  Accessed via
                ``_cfg_get`` so both ``cfg["key"]`` and ``cfg.key`` work.
            git_status_repos: List of file paths whose git repositories
                should be logged.  Defaults to ``[rsl_rl.__file__]``.
            num_steps_per_env: Override for ``num_steps_per_env`` (useful
                when *cfg* does not contain this key).  Falls back to
                ``_cfg_get(cfg, "num_steps_per_env", 0)``.
        """
        ...
        # Create RND buffers
        rnd_cfg = _cfg_get(cfg, "algorithm", {})
        if isinstance(rnd_cfg, dict):
            rnd_cfg = rnd_cfg.get("rnd_cfg") if "rnd_cfg" in rnd_cfg else _cfg_get(rnd_cfg, "rnd_cfg", None)
        else:
            rnd_cfg = _cfg_get(rnd_cfg, "rnd_cfg", None)
        if rnd_cfg:
    ...
    def init_logging_writer(
        self,
        wandb_define_metrics_callback: Callable[[], None] | None = None,
    ) -> None:
        ...
        Args:
            wandb_define_metrics_callback: Optional callback that is invoked
                after the wandb writer is initialized (used to call
                ``wandb.define_metric`` for custom evaluation metrics).
        """
        if self.log_dir is not None and not self.disable_logs:
            self.logger_type = _cfg_get(self.cfg, "logger", "tensorboard")
            ...
            if self.logger_type == "neptune":
                ...
                # Optional: allow caller to register custom wandb metrics
                if wandb_define_metrics_callback is not None:
                    wandb_define_metrics_callback()
    ...
    def log(
        ...
        Args:
            timing_details: Optional dict with keys ``env_step_time``,
                ``action_time``, ``process_time``, ``simulation_time``
                (all in seconds).  When provided, detailed Perf/ timing
                breakdown is logged.
            metrics: Optional dict of non-loss metrics.  Each key is logged
                under ``Metrics/<key>``.
        """
        if self.writer is not None:
            collection_size = self.num_steps_per_env * self.num_envs * self.gpu_world_size
                ...
                if key == "metrics":
                    ...
                    for metric_name, metric_value in value.items():
                        if isinstance(metric_value, (int, float)):
                            self.writer.add_scalar(f"Metrics/{metric_name}", metric_value, it)
                elif isinstance(value, (int, float)):
                    self.writer.add_scalar(f"Loss/{key}", value, it)
            self.writer.add_scalar("Loss/learning_rate", learning_rate, it)

            # Log optional standalone metrics
            if metrics:
            ...
            # Log detailed timing breakdown if provided
            if timing_details is not None:
                for timing_key in ("env_step_time", "action_time", "process_time", "simulation_time"):
                    if timing_key in timing_details:
                        self.writer.add_scalar(f"Perf/{timing_key}", timing_details[timing_key], it)

                # Calculate percentages
                total_time = collect_time + learn_time
                sim_time = timing_details.get("simulation_time", 0.0)
                sim_percentage = (sim_time / total_time * 100) if total_time > 0 else 0
                opt_percentage = (learn_time / total_time * 100) if total_time > 0 else 0
                self.writer.add_scalar("Perf/simulation_percentage", sim_percentage, it)
                self.writer.add_scalar("Perf/optimization_percentage", opt_percentage, it)

            # Log rewards and episode length
            if len(self.rewbuffer) > 0:
                rnd_active = hasattr(self, "erewbuffer") and hasattr(self, "irewbuffer")
                if rnd_active:
                ...
                if self.logger_type != "wandb":
                    ...
                    self.writer.add_scalar("Train/mean_episode_length/time", statistics.mean(self.lenbuffer), int(self.tot_time))
            ...
            run_name = _cfg_get(self.cfg, "run_name", None)
            ...
            eta = self.tot_time / done_it * remaining_it if done_it > 0 else 0
            log_string += (
                f"""{"-" * width}\n"""
