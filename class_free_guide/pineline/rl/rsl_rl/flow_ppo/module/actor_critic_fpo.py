...
class ActorCritic(nn.Module):
    ...
    def __init__(
        ...
        if cfg.actor_final_layer_weight_scale is not None and cfg.actor_final_layer_weight_scale != 1.0:
            final_layer = self.actor[-1]
            assert isinstance(final_layer, nn.Linear), "Expected final layer to be Linear"
            with torch.no_grad():
                final_layer.weight.data *= cfg.actor_final_layer_weight_scale
                if final_layer.bias is not None:
                    final_layer.bias.data *= cfg.actor_final_layer_weight_scale
            print(f"Applied actor_final_layer_weight_scale={cfg.actor_final_layer_weight_scale} to final layer")
    ...
    def act(self, observations: torch.Tensor, **kwargs):
        ...
        assert len(observations.shape) == 2, "observations should be of shape (batch_size, obs_dim)"
        ...
        x_t = self._compiled_integrate_flow(observations, x_t, t_current, dt, flow_steps)
    ...
    def get_cfm_loss(
        ...
        assert len(observations.shape) == 2, "observations should be of shape (batch_size, obs_dim)"
        assert observations.shape[0] == batch_dims, "actor_obs and actions should have the same batch size"
        ...
        actor_obs_expanded = observations[:, None, :].expand(batch_dims, n_samples_per_action, -1)
