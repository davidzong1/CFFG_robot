...
class UniformVelocityCommand(CommandTerm):
    ...
    def _update_metrics(self) -> None:
        ...
        self.metrics["error_vel_xy"] += torch.norm(self.vel_command_b[:, :2] - self.robot.data.root_link_lin_vel_b[:, :2], dim=-1) / max_command_step
        self.metrics["error_vel_yaw"] += torch.abs(self.vel_command_b[:, 2] - self.robot.data.root_link_ang_vel_b[:, 2]) / max_command_step
    ...
    def _resample_command(self, env_ids: torch.Tensor) -> None:
        ...
        self.vel_command_b[env_ids, :] *= torch.where(
            ...
            torch.ones_like(self.vel_command_b[env_ids, 0]),
